layui.define(['table', 'jquery', 'form', 'myXmSelect'], function (exports) {
    "use strict";

    var MOD_NAME = 'tableSelect',
        $ = layui.jquery,
        table = layui.table,
        myXmSelect = layui.myXmSelect,
        form = layui.form;
    ...
    tableSelect.prototype.render = function (opt) {
        ...
        opt.needsearchKey2 = opt.needsearchKey2 || false;
        opt.searchKey2 = opt.searchKey2 || 'keyword2';
        opt.search2Placeholder = opt.search2Placeholder || '关键词搜索';
        opt.hasSearchCategory = opt.hasSearchCategory || false;
        opt.hasSearchType = opt.hasSearchType || false;
        opt.searchPlaceholder = opt.searchPlaceholder || '关键词搜索';
        opt.selectPlaceholder = opt.selectPlaceholder || false;
        opt.selectPlaceholderData = opt.selectPlaceholderData || [];
        ...
        elem.off('click').on('click', function (e) {
            ...
            if (opt.hasSearchCategory) {
                tableBox += '<div style="display:inline-block; padding-left: 0px;width: 190px" id="' + opt.searchCategoryID + '" name="' + opt.searchCategoryKey + '" placeholder="' + opt.searchCategoryPlaceholder + '" autocomplete="off" class="layui-input layui-input-inline"></div>';
            }
            if (opt.hasSearchType) {
                tableBox += '<div style="display:inline-block; padding-left: 0px;width: 190px" id="' + opt.searchTypeID + '" name="' + opt.searchTypeyKey + '" placeholder="' + opt.searchTypePlaceholder + '" autocomplete="off" class="layui-input layui-input-inline"></div>';
            }
            tableBox += '<input style="display:inline-block;width:190px;height:38px;vertical-align:middle;margin-right:-1px;border: 1px solid #C9C9C9;" type="text" name="' + opt.searchKey + '" placeholder="' + opt.searchPlaceholder + '" autocomplete="off" class="layui-input">';
            if (opt.needsearchKey2) {
                tableBox += '<input style="display:inline-block;width:190px;height:38px;vertical-align:middle;margin-right:-1px;border: 1px solid #C9C9C9;" type="text" name="' + opt.searchKey2 + '" placeholder="' + opt.search2Placeholder + '" autocomplete="off" class="layui-input">';
            }
            if (opt.selectPlaceholder) {
                tableBox += '<select name="type" style="display:inline-block;width:190px;height:38px;vertical-align:middle;margin-right:-1px;border: 1px solid #C9C9C9;"> <option value="">选择题型</option>';

                for (var i = 0; i < opt.selectPlaceholderData.length; i++) {
                    tableBox += '<option value="' + opt.selectPlaceholderData[i].value + '">' + opt.selectPlaceholderData[i].name + '</option>';
                }
                tableBox += '</select>';
            }

            tableBox += '<button class="layui-btn layui-btn-sm layui-btn-primary tableSelect_btn_search" lay-submit lay-filter="tableSelect_btn_search" style="height: 38px"><i class="layui-icon layui-icon-search"></i></button>';
            tableBox += '</form>';
            ...
            if (opt.hasSearchCategory) {
                //渲染多选
                myXmSelect.render(opt.searchCategoryConfig);
            }
            if (opt.hasSearchType) {
                //渲染多选
                myXmSelect.render(opt.searchTypeConfig);
            }
            //数据缓存
            var checkedData = [];
            ...
            if( window.frames.length != parent.frames.length){
                ...
                overHeight && tableBox.css({'top': 'auto', 'bottom': (0 - $(window).scrollTop())+'px'});
            ...
            tableResize(window);
            $(window).resize(function(){tableResize(this)});
            function tableResize(obj) {
                var width = $(obj.document).find('.layui-fluid').width();
                $(obj.document).find('.tableSelect').css({
                    left: width * 0.02,
                    minWidth: width * 0.96,
                    width: width * 0.96,
