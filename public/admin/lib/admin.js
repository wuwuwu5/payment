...
layui.define(['view', 'md5','cacheNav'], function (exports) {
    var $ = layui.jquery
        , laytpl = layui.laytpl
        ...
        , admin = {
            ...
            , closeThisTabs: function () {
                ...
                var itemObj=$("#LAY_app_tabsheader li").eq(admin.tabsPage.index);
    ...
    element.on('tab(' + FILTER_TAB_TBAS + ')', function (data) {
        ...
        //取得当前的url
        var on_url=$(this).attr('lay-id');

        var key = md5.md5(on_url);
        localStorage.setItem("admin_on_nav", (key));
        admin.tabsPage.index = data.index;
    });
    $(document).on('dblclick', '#LAY_app_tabsheader li', function () {
        //console.log('双击刷新当前页面');
        var ELEM_IFRAME = '.layadmin-iframe'
            , length = $('.' + TABS_BODY).length;
