...
@section('content')
    <div class="layui-tab layui-tab-brief" lay-filter="docDemoTabBrief">
        <ul class="layui-tab-title">
            ...
            <li style="display: none">Themify Icons</li>
        </ul>
...
@push('scripts')
    <script>
        ...
        layui.use(['index'], function () {
            ...
            $(".site-doc-icon li, .icon-container").click(function(){
                var icon_class=$(this).find(".doc-icon-fontclass").text();
                var getFrames=$(parent.document).find(".layui-layer-content iframe");//获取页面中的iframe
                var brotherFrame=getFrames[0];//获取刷新的iframe
                var divObj = $(brotherFrame.contentWindow.document).find(to_obj);
                divObj.find('input').val('layui-icon '+icon_class);
                divObj.find(".icon-add").empty().append('<span class="layui-icon '+icon_class+'"></span>');
